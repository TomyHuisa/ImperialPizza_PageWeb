...
test("Interfaz indicador de puntos totales en cuenta", async ({ page }) => {
  ...
  await page
    .getByRole("button", { name: "Pizzas Clásicas Las favoritas" })
    .click();
  await page
    .getByRole("button", { name: "Pizzas Especiales Nuestras" })
    .click();
  await page
    .getByRole("button", { name: "Pizzas Vegetarianas Sabores" })
    .click();
  await page
    .getByRole("button", { name: "Personaliza Crea tu propia" })
    .click();
